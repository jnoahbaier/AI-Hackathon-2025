...
class GeminiTranscriptionService {
  constructor() {
    if (
      ...
      console.warn(
        '⚠️ GEMINI_API_KEY not set. Please add your Gemini API key to .env file'
      );
      this.genAI = null;
    } else {
  ...
  async transcribeAudio(audioFilePath, retries = 3) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.log(
          `🎙️ Transcribing audio file with Gemini: ${path.basename(
            audioFilePath
        ...
        console.log(
          `✅ Gemini transcription completed for ${path.basename(
            audioFilePath
          )}`
        );
        console.log(
        ...
        console.error(
          `❌ Gemini transcription error (attempt ${attempt}/${retries}):`,
          error.message
        );
